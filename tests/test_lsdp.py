...
class TestLSDPDiscovery:
    ...
    @patch('socket.socket')
    def test_discover_with_devices(self, mock_socket):
        ...
        # Should extract IP from packet
        assert len(result) >= 0  # May or may not parse correctly depending on packet structure
